...
const urls: Array<string> = reactive(
  localStorage.getItem('urls') ? JSON.parse(localStorage.getItem('urls')!) : []
);
const index = ref(
  localStorage.getItem('urlIndex') &&
    JSON.parse(localStorage.getItem('urlIndex')!) >= 0 &&
    JSON.parse(localStorage.getItem('urlIndex')!) < urls.length
    ? JSON.parse(localStorage.getItem('urlIndex')!)
    : 0
);
    ...
    <div v-if="connection.getMode() == 'mock'">
      {{
        connection.getStatus().value == 'error'
          ? 'MOCK'
          : connection.getStatus().value.toUpperCase() + ' (DEV)'
      }}
    </div>
    <div v-else>
      {{ connection.getStatus().value.toUpperCase() }}
      <span style="text-decoration: underline"
        >{{ connection.getName().value.toUpperCase() }}
      </span>
