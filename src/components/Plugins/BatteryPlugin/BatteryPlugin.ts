...
export default class BatteryPlugin extends ECTSPlugin {
    ...
    constructor() {
        ...
        this.data.set("ects/battery/usage", { percentage: 0.15, voltage: 12.7, power_supply_health: sensor_msgs.POWER_SUPPLY_HEALTH_COLD, power_supply_status: sensor_msgs.POWER_SUPPLY_STATUS_CHARGING, power_supply_technology: sensor_msgs.POWER_SUPPLY_TECHNOLOGY_LIPO, charge: 2.2, current: 3.5, capacity: 20.2, design_capacity: 20.8 } as sensor_msgs.BatteryState);
        this.data.set("ects/battery/is_critical", { data: true } as std_msgs.Bool);
        this.data.set("ects/battery/estimated_time_remaining", { data: Infinity } as std_msgs.Float32);
    }
