...
const wifiSignalStrength = computed(() => {
    ...
    const usage = props.refs.get(`/ects/system/network/${wifi_adapter_name}/usage`) as ects_msgs.NetworkUsage;
...
const wifiSsid = computed(() => {
    ...
    const wifi_adapter_name = network_adapters.adapters.find((adapter: string) => adapter.startsWith('w'));
    if (!wifi_adapter_name) return null;
    const info = props.refs.get(`/ects/system/network/${wifi_adapter_name}/info`) as ects_msgs.NetworkInfo;
