            ...
            this.aggregationList.forEach((aggregation) => {
                ...
                this.ects.callService("/ects/system/network/adapters", "ects/NetworkAdapters", new ROSLIB.ServiceRequest({})).then((response) => {
                    ...
                    this.networkAdapters.forEach((adapter) => {
                        ...
                        ects.registerListener(this, `/ects/system/network/${adapter}/info`, 'ects/NetworkInfo');
                    });
                });
                this.ects.callService("/ects/system/disk/mountpoints", "ects/MountpointList", new ROSLIB.ServiceRequest({})).then((response) => {
                    this.disks = (response as ects_msgs.MountpointList).mountpoint.map((disk) => disk.split("/").join("").split(".").join(""));
