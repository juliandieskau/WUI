    ...
    datasets: [
      {
        label: 'CPU',
        data: props.refs
          .get('/ects/system/cpu/usage')
          .per_core_usage.map((usage: number) => usage * 100),
    ...
    datasets: [
      {
        ...
        ).measurements.map((usage: ects_msgs.NetworkUsage) => usage.down_speed),
      ...
      },
      {
        ...
        ).measurements.map((usage: ects_msgs.NetworkUsage) => usage.up_speed),
...
const totalDiskUsage = computed(() => {
  return totalDiskUsed.value && totalDiskSpace.value
    ? ((totalDiskUsed.value / totalDiskSpace.value) * 100).toFixed(2)
    : null;
});
</script>
